...
class TestCommentCreation(TestCase):
    ...
    def test_user_can_create_comment(self):
        """
        Залогиненный пользователь может оставить комментарий: отправим
        POST-запрос через авторизованный клиент.
        Поля комментария содержат корректную информацию.
        """
...
class TestCommentEditDelete(TestCase):
    ...
    COMMENT_TEXT = 'Текст сомментария'
    ...
    def setUpTestData(cls):
        ...
        cls.news = News.objects.create(title='Заголовок', text='Текст')
        # Формируем адресс блока с комментариями, который понадобиться для
        # тестов.
        # Адрес новости.
        news_url = reverse('news:detail', args=(cls.news.id,))
        #   Адрес блока с коммунтариями.
        cls.url_to_comments = news_url + '#comments'
        #   Адрес страницы с новостью.
        cls.url = reverse('news:detail', args=(cls.news.id,))
        # Создаём пользователя - автора комментариея.
        cls.author = User.objects.create(username='Автор комментария')
        # Создаем клиента для пользвателя-автора коменнтария.
    ...
    def test_author_can_delete_comment(self):
        ...
        # Сосчитаем количество коментариев в системе.
        comments_count = Comment.objects.count()
        # Ожидаем ноль коментариев в системе.
        self.assertEqual(comments_count, 0)

    def test_user_cant_delete_comment_of_another_user(self):
        """Пользователь не может удалить чужой комментарий."""
        # Выполняем запрос на удаления пользователя-читателя.
